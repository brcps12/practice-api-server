...
def appeach_mansion():
    ...
    open(path.join(path.dirname(__file__), 'data/p0.in'), 'w').write('\n'.join(data))


def jayg_building():
    data = gen(pr=[0.025, 0.025, 0.95], count=200, min_floor=1, max_floor=25, seconds=100, mean=2.0, var=5.0)
    open(path.join(path.dirname(__file__), 'data/p1.in'), 'w').write('\n'.join(data))
...
def ryan_tower():
    ...
    open(path.join(path.dirname(__file__), 'data/p2.in'), 'w').write('\n'.join(data))

def load_dataset(idx):
    filename = path.join(path.dirname(__file__), 'data/p{}.in'.format(idx))
